...
def resolve_provider_client(
    ...
    if provider != "auto" and not _is_aux_provider_enabled(provider):
        logger.info("Auxiliary provider %s skipped because providers.%s.enabled is false", provider, provider)
        return None, None
...
def _get_cached_client(
    ...
    with _client_cache_lock:
        if cache_key in _client_cache:
            ...
            effective_provider = _effective_provider_for_client(cached_client, _normalize_aux_provider(provider))
            # An openrouter.ai-backed client is governed by providers.openrouter.enabled whatever label it
            # was cached under (upstream dropped the _is_openrouter_client helper; the host check stays).
            if any(base_url_host_matches(str(getattr(obj, "base_url", "") or ""), "openrouter.ai")
                   for obj in (cached_client, getattr(cached_client, "_client", None), getattr(cached_client, "client", None)) if obj):
                effective_provider = "openrouter"
            if not _is_aux_provider_enabled(effective_provider):
                # A provider can be disabled while the gateway is running: drop the cache reference so no
                # new auxiliary call reuses it. Do not close — another caller may own an in-flight request.
