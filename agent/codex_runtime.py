...
def _record_codex_app_server_usage(agent, turn, messages=None, *, api_latency_ms: float = 0.0) -> dict[str, Any]:
    ...
    def billing(**extra):
        # task_id/turn_id/api_latency_ms feed the per-task cost/outcome insights (local telemetry).
        return dict(model=agent.model, billing_provider=agent.provider, billing_base_url=agent.base_url, api_call_count=1,
                    task_id=getattr(agent, "_current_task_id", None), turn_id=getattr(agent, "_current_turn_id", None),
                    api_latency_ms=api_latency_ms, **extra)
...
def _finish_codex_turn(agent, turn, messages: List[Dict[str, Any]], *, original_user_message: Any,
                       should_review_memory: bool, api_latency_ms: float = 0.0) -> dict[str, Any]:
    ...
    usage_result = _record_codex_app_server_usage(agent, turn, messages=messages, api_latency_ms=api_latency_ms)
