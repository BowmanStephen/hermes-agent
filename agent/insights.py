...
class InsightsEngine:
    ...
    def get_usage_breakdown(self, days: int = 30, source: str = None) -> Dict[str, Any]:
        ...
        return {"tools": self._compute_tool_breakdown(self._get_tool_usage(cutoff, source)),
                ...
                "tasks": self._compute_task_telemetry(self._get_task_telemetry(cutoff, source))}
    ...
    # Per-task turn ledger (local telemetry; see hermes_state task_telemetry). The JOIN
    # scopes by session so the ``source`` filter and ``days`` window match the other queries.
    _GET_TASK_TELEMETRY_ALL, _GET_TASK_TELEMETRY_WITH_SOURCE = _scoped(
        "SELECT t.* FROM task_telemetry t JOIN sessions s ON s.id = t.session_id WHERE s.started_at >= ?",
        " ORDER BY t.started_at DESC",
    )

    def _get_task_telemetry(self, cutoff: float, source: str = None) -> List[Dict]:
        """Fetch task-turn telemetry, tolerating pre-telemetry databases (no table -> empty)."""
        try:
            return [dict(row) for row in self._query("_GET_TASK_TELEMETRY", cutoff, source)]
    ...
    def _compute_task_telemetry(rows: List[Dict]) -> Dict[str, Any]:
        ...
        if not rows:
            return {
                "summary": {"turns": 0, "tasks": 0, "completed": 0, "failed": 0, "interrupted": 0, "partial": 0,
                            "estimated_cost_usd": 0.0, "actual_cost_usd": 0.0, "avg_latency_ms": 0.0,
                            "avg_quality_score": None, "budget_alerts": 0},
                "by_task": [], "recent": [],
            }
        grouped: Dict[str, Dict[str, Any]] = {}
        quality_values: List[float] = []
        summary_counts: Counter = Counter()
        total_estimated = total_actual = total_latency = 0.0
        latency_count = budget_alerts = 0
        for row in rows:
            task_id = str(row.get("task_id") or "unknown")
            group = grouped.setdefault(task_id, {
                "task_id": task_id, "turns": 0, "input_tokens": 0, "output_tokens": 0,
                "estimated_cost_usd": 0.0, "actual_cost_usd": 0.0, "latency_ms": 0.0,
                "outcomes": Counter(), "models": set(), "budget_alerts": 0, "last_seen": row.get("started_at"),
            })
            ...
            est, act = float(row.get("estimated_cost_usd") or 0.0), float(row.get("actual_cost_usd") or 0.0)
            group["estimated_cost_usd"] += est
            group["actual_cost_usd"] += act
            total_estimated += est
            total_actual += act
        ...
        by_task = [{
            "task_id": g["task_id"], "turns": g["turns"], "input_tokens": g["input_tokens"], "output_tokens": g["output_tokens"],
            "total_tokens": g["input_tokens"] + g["output_tokens"],
            "estimated_cost_usd": g["estimated_cost_usd"], "actual_cost_usd": g["actual_cost_usd"],
            "avg_latency_ms": g["latency_ms"] / g["turns"] if g["turns"] else 0.0,
            "outcomes": dict(g["outcomes"]), "models": sorted(g["models"]),
            "budget_alerts": g["budget_alerts"], "last_seen": g["last_seen"],
        } for g in grouped.values()]
        by_task.sort(key=lambda item: item["estimated_cost_usd"], reverse=True)
        return {
            "summary": {
                "turns": len(rows), "tasks": len(grouped),
                "completed": summary_counts.get("completed", 0), "failed": summary_counts.get("failed", 0),
                "interrupted": summary_counts.get("interrupted", 0), "partial": summary_counts.get("partial", 0),
                "estimated_cost_usd": total_estimated, "actual_cost_usd": total_actual,
                "avg_latency_ms": total_latency / latency_count if latency_count else 0.0,
                "avg_quality_score": sum(quality_values) / len(quality_values) if quality_values else None,
                "budget_alerts": budget_alerts,
            },
            "by_task": by_task, "recent": rows[:20],
        }
    ...
    def format_terminal(self, report: Dict) -> str:
        ...
        ts = (report.get("tasks") or {}).get("summary") or {}
        if ts.get("turns", 0) > 0:
            lines += self._section("🎯 Task Telemetry") + [
                f"  Turns:              {ts['turns']:<12}  Tasks:           {ts['tasks']:,}",
                f"  Completed:          {ts['completed']:<12}  Failed:          {ts['failed']:,}",
                f"  Estimated task cost: {_fmt_est_cost(ts['estimated_cost_usd'])}",
            ]
            if ts.get("budget_alerts", 0):
                lines.append(f"  Budget alerts:      {ts['budget_alerts']:,}")
            lines.append("")
        if report["models"]:
    ...
    def format_gateway(self, report: Dict) -> str:
        ...
        ts = (report.get("tasks") or {}).get("summary") or {}
        if ts.get("turns", 0) > 0:
            task_line = f"**🎯 Tasks:** {ts['completed']}/{ts['turns']} completed | {_fmt_est_cost(ts['estimated_cost_usd'])}"
            if ts.get("budget_alerts", 0):
                task_line += f" | ⚠ {ts['budget_alerts']} budget alert(s)"
            lines += [task_line, ""]
        if report["models"]:
