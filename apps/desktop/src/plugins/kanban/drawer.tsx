...
type FeedTab = 'activity' | 'comments' | 'log' | 'runs'
 ...
 *  The tab is owned by the drawer so a diagnostic's `comment` action can
 *  surface the composer from another tab; `composerRef` reaches the comment
 *  textarea for the same handoff, and `logError` keeps a failing worker-log
 *  fetch from reading as "no log". */
function FeedTabs({
  commentPending,
  composerRef,
  detail,
  ...
  const hasLog = !!logError || (!!log?.exists && !!log.content)
  ...
  const body = (
    <div className="flex flex-col gap-4">
      ...
      {tab === 'log' &&
        (logError ? (
          // A failing log fetch must not read as "the worker printed nothing" —
          // the opposite of the truth when the backend is unreachable.
          <p className="text-[0.6875rem] leading-relaxed text-destructive">{k.workerLogError(errText(logError))}</p>
        ) : (
  ...
  return (
    <section className="flex flex-col gap-3">
      <div className="flex items-center justify-between gap-2">
        <SegmentedControl onChange={onTabChange} options={tabs} value={tab} />
        {help}
      </div>
      {body}
    </section>
...
export function TaskDrawer({
  ...
  // A diagnostic's `comment` action means "answer this here" — surface the
  // composer (switching to Comments if another feed tab is showing) and put
  // the caret in it. The focus waits a frame so a tab switch can mount the
  // textarea first.
  const focusComposer = () => {
    setFeedTab('comments')
