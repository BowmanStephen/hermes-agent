"""Gateway agent execution runtime service."""
...
def _run_helper(name: str, *args: Any, **kwargs: Any) -> Any:
    from gateway import run as _run

    return getattr(_run, name)(*args, **kwargs)


def _auto_continue_freshness_window() -> float:
    return _run_helper("_auto_continue_freshness_window")


def _build_media_placeholder(event: Any) -> str:
    return _run_helper("_build_media_placeholder", event)


def _build_replay_entry(role: str, content: Any, msg: Dict[str, Any]) -> Dict[str, Any]:
    return _run_helper("_build_replay_entry", role, content, msg)


def _dequeue_pending_event(adapter: Any, session_key: str) -> Any:
    return _run_helper("_dequeue_pending_event", adapter, session_key)


def _float_env(name: str, default: float) -> float:
    return _run_helper("_float_env", name, default)


def _is_control_interrupt_message(message: Optional[str]) -> bool:
    return _run_helper("_is_control_interrupt_message", message)


def _is_fresh_gateway_interruption(*args: Any, **kwargs: Any) -> bool:
    return _run_helper("_is_fresh_gateway_interruption", *args, **kwargs)


def _last_transcript_timestamp(history: Optional[List[Dict[str, Any]]]) -> Any:
    return _run_helper("_last_transcript_timestamp", history)


def _load_gateway_config() -> dict:
    return _run_helper("_load_gateway_config")


def _platform_config_key(platform: Any) -> str:
    return _run_helper("_platform_config_key", platform)


def _prepare_gateway_status_message(platform: Any, event_type: str, message: str) -> Optional[str]:
    return _run_helper("_prepare_gateway_status_message", platform, event_type, message)


def _preserve_queued_followup_history_offset(current_result: dict, followup_result: dict) -> dict:
    return _run_helper("_preserve_queued_followup_history_offset", current_result, followup_result)


def _redact_gateway_user_facing_secrets(text: str) -> str:
    return _run_helper("_redact_gateway_user_facing_secrets", text)


def _reload_runtime_env_preserving_config_authority() -> None:
    _run_helper("_reload_runtime_env_preserving_config_authority")


def _resolve_gateway_model() -> str:
    return _run_helper("_resolve_gateway_model")


class GatewayAgentExecutionRuntime:
    def __init__(
        self,
        runner: Any,
        *,
        hermes_home: Any,
        interrupt_reason_timeout: str,
    ) -> None:
        object.__setattr__(self, "_runner", runner)
        object.__setattr__(self, "_hermes_home", hermes_home)
        object.__setattr__(self, "_interrupt_reason_timeout", interrupt_reason_timeout)

    def __getattr__(self, name: str) -> Any:
        return getattr(self._runner, name)

    def __setattr__(self, name: str, value: Any) -> None:
        if name in {"_runner", "_hermes_home", "_interrupt_reason_timeout"}:
        ...
        try:
            ...
            if _inactivity_timeout:
                ...
                if _timed_out_agent and hasattr(_timed_out_agent, "interrupt"):
                    _timed_out_agent.interrupt(self._interrupt_reason_timeout)
