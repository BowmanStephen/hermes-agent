...
def _resolve_dispatcher_settings(kanban_cfg: dict, kb: Any) -> _DispatcherSettings:
    ...
    # Read kanban.default_max_runtime_seconds — fleet-wide runtime backstop
    # for tasks with no per-task max_runtime_seconds (which is almost all of
    # them). Without it enforce_max_runtime skips those rows entirely, so a
    # worker that keeps heartbeating while making no progress is bounded only
    # by the 1-hour heartbeat backstop. A per-task value always wins. 0/unset
