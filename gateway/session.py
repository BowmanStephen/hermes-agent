...
class SessionStore(
    ...
    def _get_or_create_session_impl(
        ...
        self._drop_missing_parent_lineage(session_key, create_kwargs)
    ...
    def _drop_missing_parent_lineage(
        self, session_key: str, create_kwargs: Optional[Dict[str, Any]],
    ) -> None:
        """The JSON routing index can outlive its SQLite predecessor row (partial state restore,
        legacy DB repair, FK races on reset). Null ``parent_session_id`` / ``model_config`` when the
        predecessor row is absent so the INSERT cannot manufacture a dangling self-referential FK
        and the new row stays usable as a root session. A transient read failure is treated the
        same way: it must not turn a user-requested reset into a guaranteed FK failure — the next
        peer refresh restores routing metadata once the DB is available."""
        if not create_kwargs:
            return
        parent_session_id = create_kwargs.get("parent_session_id")
        db = self._db_for_key(session_key)
        if not db or not parent_session_id:
            return
        try:
            get_session = getattr(db, "get_session", None)
            if not callable(get_session) or get_session(parent_session_id) is not None:
                return
        except Exception:
            pass
        create_kwargs["parent_session_id"] = None
        create_kwargs["model_config"] = None
    ...
    def reset_session(self, session_key: str, display_name: Optional[str] = None) -> Optional[SessionEntry]:
        ...
        self._drop_missing_parent_lineage(session_key, db_create_kwargs)
        self._finish_route_transition(
