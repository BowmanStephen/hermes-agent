...
def _kill_stale_dashboard_processes(
    ...
    macOS: a PID supervised by a launchd job (LaunchAgent / LaunchDaemon, matched against the
    loaded jobs via ``main_dashboard._launchd_job_owning_backend``) is handed back to launchd via
    ``main_dashboard._restart_launchd_job`` (a ``launchctl kickstart`` plus fresh-PID check)
    instead of being respawned detached — KeepAlive would otherwise relaunch the plist copy into a
    port the detached twin already holds (the Aug-2026 ~19.7h EADDRINUSE loop).
