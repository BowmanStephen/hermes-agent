...
def _launchd_label_for_pid(pid: int) -> str | None:
    ...
    launchd stamps ``XPC_SERVICE_NAME=<label>`` on every process it spawns, while interactive
    shells inherit the sentinel ``"0"`` (same probe as the gateway handback, #86893). Recent
    macOS builds dropped ``ps -E`` and suppress ``ps e`` environment disclosure, so when the env
    is unreadable we fall back to the pid->label table in ``launchctl print gui/<uid>``.
    """
    if sys.platform != "darwin":
        return None
    for argv in (["ps", "-EWW", "-p", str(pid)], ["ps", "eww", str(pid)]):
        try:
            result = subprocess.run(argv, timeout=10, **_PS_RUN_KWARGS)
        except (FileNotFoundError, subprocess.TimeoutExpired, OSError):
            continue
        if result.returncode != 0:
            continue
        ...
        for token in stdout.split():
            if token.startswith("XPC_SERVICE_NAME="):
                raw = token.split("=", 1)[1]
                if raw == "0":
                    # Interactive-shell sentinel: env readable, process not launchd-managed.
                    return None
                # Keep the segment before any ":" and strip any "/" path prefix so kickstart gets
                # the bare job label ("gui/501/ai.hermes.dashboard" -> "ai.hermes.dashboard").
                label = raw.split(":", 1)[0]
                if "/" in label:
                    label = label.rsplit("/", 1)[-1]
                return label or None
        if "=" in stdout:
            # Environment fully disclosed and XPC_SERVICE_NAME absent: genuinely not launchd-managed.
            return None
    # Env unreadable (macOS 26+): match the pid against the user's launchd domain listing, whose
    # job table reads "<pid> <last-status> <label>".
    try:
        result = subprocess.run(
            ["launchctl", "print", f"gui/{os.getuid()}"], timeout=10, **_PS_RUN_KWARGS)
    except (FileNotFoundError, subprocess.TimeoutExpired, OSError):
        return None
...
def _kill_stale_dashboard_processes(
    ...
    macOS: a PID supervised by a launchd LaunchAgent (``XPC_SERVICE_NAME`` in its environment, else a
    job in the user's ``gui`` launchd domain) is kickstarted in place via
    ``launchctl kickstart -k gui/<uid>/<label>`` instead of being respawned detached — KeepAlive would
    otherwise relaunch the plist copy into a port the detached twin already holds.
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    ...
    if restart_managed and sys.platform != "win32":
        for pid in pids:
            ...
            if not pid_service[pid]:
                if cmdline := _dash._dashboard_cmdline_for_pid(pid):
                    # Manual process: exact argv + HERMES_HOME for the respawn and its profile cap.
                    # Manually-started process: preserve its exact argv so we can respawn it after the
                    # update (#40449, #68934). Snapshot HERMES_HOME before the kill so per-profile caps
                    # still work after the process is gone (#78821).
                    pid_cmdline[pid] = cmdline
                    pid_home[pid] = _hermes_home_for_pid(pid)
                # Snapshot any launchd job label before the kill too: the pid's env / launchd domain
                # entry must be read while the process is still alive (macOS handback below).
    ...
    if killed and restart_managed:
        unrecovered = _restart_killed_backends(
            killed, pid_service, pid_cgroup, pid_cmdline, pid_home, pid_launchd)
    else:
...
def _restart_killed_backends(
    ...
    pid_launchd: dict[int, str | None] | None = None) -> list[int]:
    ...
    for pid in killed:
        ...
        if svc_name:
        ...
        elif label := (pid_launchd or {}).get(pid):
            # macOS launchd handback: without it, `hermes update` SIGTERMed the launchd-managed
            # dashboard and respawned a detached twin; launchd's KeepAlive then relaunched the plist
            # copy into a port the twin already held, a permanent EADDRINUSE loop (2,359 "[Errno 48]
            # address already in use" errors, ~19.7h, Aug 2026). Kickstart the owning job so launchd
            # restarts it in place instead.
            ...
            if _kickstart_launchd_label(label):
                ...
                failed_restarts.append((label, "launchctl kickstart returned non-zero"))
                unrecovered.append(pid)
