...
def _launch_browser_probe(timeout: float) -> tuple:
    ...
    Drives the same agent-browser CLI as Hermes' browser tool. Do NOT import Python Playwright here: it is
    not a Hermes dependency, so installations on the supported agent-browser backend would be reported
    ...
    try:
        ...
        browser_cmd = ["npx", "agent-browser"] if resolved == "npx agent-browser" else [resolved]
    except Exception as exc:
        return (False, f"agent-browser unavailable ({exc.__class__.__name__})")
    if not browser_cmd:
        return (False, "agent-browser command is empty")

    # A private per-PID session so this diagnostic never attaches to or closes a user's own session.
    ...
    try:
        completed = subprocess.run(open_cmd, check=False, capture_output=True, text=True, timeout=timeout)
        ...
        # Best-effort, independently bounded cleanup that never masks the useful open error.
        try:
            subprocess.run(close_cmd, check=False, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
