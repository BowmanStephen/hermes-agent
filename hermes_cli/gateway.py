...
def _reaper_exclusion_pids(extra_exclude: set | None) -> set[int] | None:
    ...
    Returns ``None`` when macOS launchd PID discovery was incomplete: the caller must skip the sweep,
    because an unknown supervised PID would otherwise be classified as an orphan and reaped."""
    ...
    try:
        ...
        # fail_closed on macOS: a transient launchctl stall must abort this destructive sweep instead of
        # exposing the root/sibling launchd gateway PID as an apparent orphan.
        own |= _get_service_pids(all_profiles=True, fail_closed=is_macos())
    except Exception:
        if is_macos():
            logger.debug("Skipping macOS orphan reap because launchd PID discovery was incomplete", exc_info=True)
            return None
