...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    Initial human-ops cards use their ``created`` event's ``status`` payload as the same durable
    signal, since they intentionally skip a separate blocked transition (fork #28712 initial
    blocked gate).
    """
    row = conn.execute(
    ...
    if row and row["kind"] == "created":
        # Initial human-ops cards: the created event's ``status`` payload is the durable
        # blocked signal — they intentionally skip a separate blocked transition
        # (fork #28712 initial blocked gate).
        try:
...
def complete_task(
    ...
    ``summary`` also fills the task-level ``result`` when no explicit ``result`` is given.
    ...
    # Board task JSON keeps the authoritative worker handoff even when only ``summary`` was passed.
    ...
    with write_txn(conn):
        ...
        sql = """
                ...
                """
        params: tuple = (task_result, now, task_id)
        if expected_run_id is not None:
