        ...
        with _kb.write_txn(conn):
            ...
            if cur.rowcount == 1:
                payload = {
                    ...
                    # The limit actually enforced — the dispatcher default
                    # when the task carries no per-task cap.
                    "limit_seconds": limit,
                    "limit_source": (
...
def _worker_board_paths(board: Optional[str]) -> tuple[Path, Path, str]:
    ...
    """
    if board is None:
        return _kb.kanban_db_path(), _kb.workspaces_root(), _kb.get_current_board()

    slug = _kb._normalize_board_slug(board) or _kb.DEFAULT_BOARD
    root = _kb.kanban_home()
