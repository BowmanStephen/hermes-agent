...
def _get_platform_tools(config: dict, platform: str, *, include_default_mcp_servers: bool = True) -> Set[str]:
    ...
    # An explicitly empty platform list is a deny-all selection. Return before composite recovery and
    # default MCP injection: otherwise non-configurable toolsets such as kanban leak back onto surfaces the
    # profile deliberately disabled. Distinct from an omitted platform key, which falls back to the composite.
