...
from agent.tool_catalog import (
    LEGACY_TOOLSET_MAP as _LEGACY_TOOLSET_MAP,
    ToolCatalogRequest,
    ToolSurface,
    build_tool_surface,
)
...
_tool_defs_cache: Dict[tuple, ToolSurface] = {}
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        if cached is not None:
            ...
            _last_resolved_tool_names = list(cached.tool_names)
            return cached.as_list()

    surface = _compute_tool_surface(enabled_toolsets, disabled_toolsets, quiet_mode)
    _last_resolved_tool_names = list(surface.tool_names)
    if quiet_mode:
        # Cache the freshly-computed surface, but hand callers a shallow copy so
        ...
        _tool_defs_cache[cache_key] = surface
        return surface.as_list()
    return surface.as_list()
...
def _compute_tool_surface(
    ...
    """Uncached deep catalog surface construction."""
    return build_tool_surface(
        ToolCatalogRequest.from_args(
