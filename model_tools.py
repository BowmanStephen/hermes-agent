...
def _select_tool_names(enabled_toolsets: Optional[List[str]], disabled_toolsets: Optional[List[str]], quiet_mode: bool) -> set:
    ...
    if (disabled_toolsets and "kanban" in disabled_toolsets and os.environ.get("HERMES_KANBAN_TASK")
            and not _is_delegated_child_context() and _is_dispatcher_owned_worker()):
        # A profile may disable the kanban toolset to hide board tools from its chat sessions, but a
        # dispatcher-spawned worker must ALWAYS keep the lifecycle handoff surface
        # (complete/block/heartbeat) — the same invariant as the enabled-append above. Without it the
        # worker can never close its task: the kanban stop-guard nudges it in a loop until the
        # iteration budget exhausts and the run records as timed_out/crashed.
