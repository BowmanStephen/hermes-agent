...
class A2AAdapter(BasePlatformAdapter):
    ...
    async def send(self, chat_id: str, content: str, reply_to: Optional[str] = None, metadata: Optional[Dict[str, Any]] = None):
        ...
        if not (metadata or {}).get("notify"):
            ...
            # Nothing was delivered and therefore nothing can be edited.  A successful result
            # makes GatewayStreamConsumer treat the ignored preview as real (even without a
            # message id); its fallback then sends only the missing tail, truncating the result.
