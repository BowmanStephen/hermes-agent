...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    # Seed platform ``extra`` too (not just the env bridge) so ``_auto_thread_enabled`` /
    # ``_reactions_enabled`` honor per-adapter YAML config; ``platform_extra_cfg`` is a fallback source.
