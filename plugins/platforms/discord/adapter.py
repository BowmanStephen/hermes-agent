...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    # Seed platform ``extra`` too (not just the env bridge) so ``_extra_or_env_flag`` honors
    # per-adapter YAML config for auto_thread / reactions; ``platform_extra_cfg`` is a fallback source.
    for key, env_key in (("auto_thread", "DISCORD_AUTO_THREAD"), ("reactions", "DISCORD_REACTIONS")):
        ...
        if value is not None:
            ...
            _env_default(env_key, str(value).lower())
