...
def test_configure_pins_platform_toolsets_per_surface(profile_dir):
    """``platform_toolsets`` pins each surface's toolset list with replace semantics and is
    reported in the configure response's ``applied`` map (fork intent, upstream shape)."""
    result = _call("profiles.configure", {
        "platform_toolsets": {"cli": ["web", "file"], "discord": ["terminal"]},
    })

    assert result["ok"] is True
    assert result["applied"] == {"platform_toolsets": True}
