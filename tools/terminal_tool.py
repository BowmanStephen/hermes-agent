...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: a kanban worker killing its own process. The dispatcher records that as
        # `crashed` and burns a retry (observed: the model reads its pid from the run record and
        # tries to "release" the task with `kill <pid>`). Finish/park semantics belong to the board tools.
        if (os.environ.get("HERMES_KANBAN_TASK") or "").strip() and contains_kanban_worker_self_kill(command):
            raise _Rejected(_error_json(
                "Blocked: this command would kill the kanban worker's own process, so the dispatcher "
                "would record this run as crashed and burn a retry. Do not kill your own pid. Finish "
                "the task with kanban_complete, or park it with kanban_block.",
