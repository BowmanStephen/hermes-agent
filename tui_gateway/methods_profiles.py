...
def _mcp_entry_enabled(entry: dict) -> bool:
    """Effective on/off of an ``mcp_servers`` entry: the canonical ``enabled`` flag AND the legacy
    ``disabled`` spelling are both honored (either one can switch a server off)."""
    from hermes_cli.tools_config import _parse_enabled_flag
    return _parse_enabled_flag(entry.get("enabled", True), default=True) and not _parse_enabled_flag(
...
def _save_mcp_toggles(cfg, enabled, launch_mcp, save_config) -> None:
    ...
    for srv, entry in mcp_cfg.items():
        if isinstance(entry, dict):
            ...
            # Normalize the legacy spelling so every reader sees the same replace-semantics result.
...
_PLATFORM_TOOLSET_SURFACES = frozenset({"cli", "discord"})


def _save_platform_toolsets(cfg, requested: dict, save_config) -> None:
    """``platform_toolsets`` (replace semantics per surface; an empty list explicitly disables every
    toolset on that surface). Only ``cli`` / ``discord`` are accepted; each value must be a list."""
    if not set(requested).issubset(_PLATFORM_TOOLSET_SURFACES):
    ...
    for platform, toolsets in requested.items():
        ...
        platform_cfg[platform] = list(dict.fromkeys(
            str(toolset).strip() for toolset in toolsets if str(toolset).strip()))
    cfg["platform_toolsets"] = platform_cfg
    save_config(cfg)


def _configure_cfg_sections(profile_dir, params, applied) -> None:
    """Apply ``disabled_skills`` / ``enabled_toolsets`` / ``platform_toolsets`` /
    ``enabled_mcp_servers`` (replace semantics; empty toolsets clears the pin, an empty
    per-surface list disables all toolsets there). An undefined MCP server is copied from the
    LAUNCH catalog (unknown names skipped); credentials stay in .env/auth."""
    ...
    with _hermes_home_scope(profile_dir):
        ...
        if isinstance(params.get("platform_toolsets"), dict):
            applied["platform_toolsets"] = _best_effort(
                lambda: _save_platform_toolsets(cfg, params["platform_toolsets"], save_config))
        if want_mcp:
...
def _(rid, params: dict) -> dict:
    ...
    ``enabled_toolsets``, ``platform_toolsets`` (``{cli|discord: [..]}``, replace semantics),
    ``enabled_mcp_servers``; sections are independent, ``applied`` reports each."""
