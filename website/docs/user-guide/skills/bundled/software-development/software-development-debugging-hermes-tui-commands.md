...
<!-- ascii-guard-ignore -->
```
Python backend (hermes_cli/commands/_registry.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ...
   search_files --pattern "commandname" --path hermes_cli/commands/_registry.py --context 3
   ```
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `hermes_cli/commands/_registry.py`. Autocomplete data ships from Python.
